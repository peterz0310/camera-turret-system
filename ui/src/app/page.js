...
const AUTO_AIM_KP = 0.55; // Increased for more responsive tracking
const AUTO_AIM_ERROR_ALPHA = 0.3; // Smoothing for stable tracking without oscillation
...
function App() {
  ...
  // Smoothed bounding box coordinates to reduce jitter
  const autoAimSmoothedBBoxRef = useRef(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        crosshairPanelRef.current &&
        !crosshairPanelRef.current.contains(event.target)
      ) {
      ...
      if (
        fpsSliderRef.current &&
        !fpsSliderRef.current.contains(event.target)
      ) {
  ...
  useEffect(() => {
    const handleKeyPress = (event) => {
      ...
      if (
        event.shiftKey &&
        (code === "Digit1" || code === "Digit2" || key === "1" || key === "2")
      ) {
    ...
    aiMode,
    currentModel,
  ]); // Added angularStepSize dependency
  ...
  const handleStreamError = () => {
    ...
    setIsStreamLoading(false);
    setStreamError(
      "Primary video feed is offline. Check camera service integrity.",
    );
  };
  ...
  const disableAutoAim = (reason) => {
    ...
    autoAimSmoothedBBoxRef.current = null; // Reset smoothed bbox
  ...
  const handleAIToggle = async () => {
    ...
    console.log(
      `🤖 [AI] Attempting to set mode to: ${newAiMode ? "ENABLED" : "DISABLED"}`,
    );
    try {
      ...
      if (response.ok && data.success) {
        ...
        console.log(
          `✅ [AI] Mode successfully set to: ${data.ai_enabled ? "ENABLED" : "DISABLED"} with model: ${data.current_model}`,
        );
      } else {
        console.error(
          "❌ [AI] Failed to toggle AI mode:",
  ...
  const handleAutoAimToggle = () => {
    ...
    if (!aiModeRef.current) {
      console.warn(
        "⚠️ [AUTO-AIM] AI detection is disabled. Enable AI Detection first.",
      );
      return;
    }
  ...
  useEffect(() => {
    if (
      autoAimEnabled &&
      (!connected || status?.calibrating || (status && !status.calibrated))
    ) {
  ...
  useEffect(() => {
    ...
    const getIntervalMs = () => {
      ...
      const modelFps =
        fpsInfo?.[activeModel]?.current_fps ||
        availableModels?.[activeModel]?.current_fps ||
        10;
    ...
    const loop = async () => {
      ...
      try {
        ...
        const detectionTimestampSec = Number.isFinite(data?.timestamp)
          ? data.timestamp
          : 0;
        const detectionTimestampMs =
          detectionTimestampSec > 0 ? detectionTimestampSec * 1000 : 0;
        ...
        if (hasFreshDetections && hasFrame) {
          ...
          if (target?.detection) {
            const [rawX1, rawY1, rawX2, rawY2] = target.detection.bbox;

            // Apply bounding box smoothing to reduce jitter from frame-to-frame variation
            // Use a separate, more aggressive smoothing for the bbox (0.5 = 50% new, 50% old)
            const bboxAlpha = 0.5;

            let x1, y1, x2, y2;
            if (!autoAimSmoothedBBoxRef.current) {
              // First detection - initialize with raw values
              x1 = rawX1;
              y1 = rawY1;
              x2 = rawX2;
              y2 = rawY2;
              autoAimSmoothedBBoxRef.current = { x1, y1, x2, y2 };
            } else {
              // Smooth the bounding box coordinates
              const prev = autoAimSmoothedBBoxRef.current;
              x1 = prev.x1 + bboxAlpha * (rawX1 - prev.x1);
              y1 = prev.y1 + bboxAlpha * (rawY1 - prev.y1);
              x2 = prev.x2 + bboxAlpha * (rawX2 - prev.x2);
              y2 = prev.y2 + bboxAlpha * (rawY2 - prev.y2);
              autoAimSmoothedBBoxRef.current = { x1, y1, x2, y2 };
            }
            ...
            const vfovRad =
              2 * Math.atan(Math.tan(hfovRad / 2) * (frameH / frameW));
            ...
            const filterReset =
              ...
              nowMs - autoAimLastDetectionRef.current >
                AUTO_AIM_DETECTION_STALE_MS;
            if (filterReset) {
              ...
              autoAimYawErrorRef.current =
                autoAimYawErrorRef.current +
                (yawErrorDeg - autoAimYawErrorRef.current) * smoothing;
              autoAimPitchErrorRef.current =
                autoAimPitchErrorRef.current +
                (pitchErrorDeg - autoAimPitchErrorRef.current) * smoothing;
            }
            ...
            if (
              Math.abs(filteredYawError) >= deadzone ||
              Math.abs(filteredPitchError) >= deadzone
            ) {
              const nowCommandMs = Date.now();
              // Use shorter interval for SNAP mode to allow continuous replanning
              const effectiveInterval = useAbsolute
                ? Math.max(intervalMs * 0.6, 100)
                : intervalMs;
              if (
                nowCommandMs - autoAimLastCommandRef.current >=
                effectiveInterval
              ) {
                ...
                const clampMinStep = (value) =>
                  Math.abs(value) < minStep ? 0 : value;
                let yawCommand = clampMinStep(yawStep) * AUTO_AIM_YAW_SIGN;
                let pitchCommand =
                  clampMinStep(pitchStep) * AUTO_AIM_PITCH_SIGN;
                ...
                const hasCommand =
                  Math.abs(yawCommand) >= minStep ||
                  Math.abs(pitchCommand) >= minStep;
                if (!hasCommand || !autoAimEnabledRef.current) {
                  return;
                }

                const angularInProgress =
                  statusRef.current?.movement?.angularInProgress;
                if (useAbsolute) {
                  // SNAP mode: Send absolute position commands for smooth tracking
                  const maxError = Math.max(
                    Math.abs(filteredYawError),
                    Math.abs(filteredPitchError),
                  );

                  // Skip if movement is in progress AND error is small OR if we're within deadzone
                  // This prevents oscillation by letting movements complete
                  ...
                  // Calculate snap movement with gain (< 1.0 for slight undershoot to prevent oscillation)
                  ...
                  snapYaw = Math.max(
                    -snapMaxDeg,
                    Math.min(snapMaxDeg, snapYaw),
                  );
                  snapPitch = Math.max(
                    -snapMaxDeg,
                    Math.min(snapMaxDeg, snapPitch),
                  );

                  // Apply deadzone to prevent tiny oscillating movements
                  // If the snap command is too small, zero it out
                  if (Math.abs(snapYaw) < deadzone * 0.5) snapYaw = 0;
                  if (Math.abs(snapPitch) < deadzone * 0.5) snapPitch = 0;

                  // If both commands are zero, we're settled - don't send anything
                  if (snapYaw === 0 && snapPitch === 0) {
                    return;
                  }

                  // Apply axis signs
                  const snapYawCommand = snapYaw * AUTO_AIM_YAW_SIGN;
                  const snapPitchCommand = snapPitch * AUTO_AIM_PITCH_SIGN;

                  let targetYaw = currentYaw + snapYawCommand;
                  let targetPitch = currentPitch + snapPitchCommand;

                  // Respect limit switches
                  if (tiltUp && targetPitch > currentPitch)
                    targetPitch = currentPitch;
                  if (tiltDown && targetPitch < currentPitch)
                    targetPitch = currentPitch;
                ...
                // Non-SNAP mode: Use relative movements with improved smoothing
                // Scale commands based on error magnitude for smoother approach
                const errorMagnitude = Math.sqrt(
                  filteredYawError ** 2 + filteredPitchError ** 2,
                );
                const scaleFactor = Math.min(1.0, errorMagnitude / 10.0); // Reduce commands when close

                const smoothYawCommand = yawCommand * (0.7 + 0.3 * scaleFactor);
                const smoothPitchCommand =
                  pitchCommand * (0.7 + 0.3 * scaleFactor);

                autoAimLastCommandRef.current = nowCommandMs;
                sendCommand({
                  moveByAngle: {
                    horizontal: smoothYawCommand,
                    vertical: smoothPitchCommand,
                  },
                });
              }
            }
  ...
  return (
    <div className="relative min-h-screen bg-[#0A192F] text-cyan-300 font-mono overflow-hidden">
      ...
      <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-30 bg-black/40 border border-cyan-500/30 px-6 py-2 rounded-sm shadow-lg backdrop-blur-sm">
        <h1 className="text-2xl font-bold text-center uppercase tracking-widest text-cyan-400">
          Sentry Turret Control
        </h1>
      </div>
      ...
      <div className="absolute top-1/2 -translate-y-1/2 left-8 z-30 flex flex-col gap-4">
        {/* Crosshair Controls */}
        <div
          ref={crosshairPanelRef}
          className="relative crosshair-panel-container pointer-events-auto"
        >
          <button
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className={crosshairOptionClass(!crosshairEnabled)}
                  >
                    SYSTEM OFF
                  </button>
                  <button
                    onClick={() => handleCrosshairToggle("crosshair-1.png")}
                    className={crosshairOptionClass(
                      crosshairEnabled &&
                        selectedCrosshair === "crosshair-1.png",
                    )}
                  >
                    Standard Cross
                  </button>
                  <button
                    ...
                    className={crosshairOptionClass(
                      crosshairEnabled &&
                        selectedCrosshair === "crosshair-2.png",
                    )}
                  >
                    Circular Dot
                  </button>
        ...
        <div className="relative pointer-events-auto">
          ...
          >
            ...
            <ChevronDown
              className={`w-4 h-4 transition-transform ${autoAimPanelOpen ? "rotate-180" : ""}`}
            />
          </button>
          {autoAimPanelOpen && (
              ...
              <div className="space-y-3 text-xs font-mono">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between p-2 bg-gray-800/50 rounded border border-gray-600">
                    ...
                    <button
                      onClick={() =>
                        setAutoAimSettings((prev) => ({
                          ...prev,
                          useAbsolute: !prev.useAbsolute,
                        }))
                      }
                  ...
                  <p className="text-[9px] text-gray-400 leading-tight">
                    SNAP: Jumps to absolute position (faster). STEP: Incremental
                    moves (smoother but slower).
                  </p>
                </div>

                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-gray-400">
                    <span className="text-cyan-400">[STEP]</span> Response (Kp):{" "}
                    {autoAimSettings.kp.toFixed(2)}
                  </label>
                  <input
                    type="range"
                    min="0.1"
                  ...
                  />
                  <p className="text-[9px] text-gray-400 leading-tight">
                    Controls how aggressively it moves. Higher = faster
                    response, may overshoot.
                  </p>
                </div>

                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-gray-400">
                    <span className="text-cyan-400">[STEP]</span> Max Step (°):{" "}
                    {autoAimSettings.maxStep.toFixed(2)}
                  </label>
                  <input
                    type="range"
                    min="0.5"
                    ...
                    onChange={(e) =>
                      setAutoAimSettings((prev) => ({
                        ...prev,
                        maxStep: parseFloat(e.target.value),
                      }))
                    }
                    className="w-full h-2 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                  />
                  <p className="text-[9px] text-gray-400 leading-tight">
                    Maximum degrees per move in STEP mode. Limits speed of
                    movement.
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-gray-400">
                    <span className="text-cyan-400">[STEP]</span> Min Step (°):{" "}
                    {autoAimSettings.minStep.toFixed(2)}
                  </label>
                  <input
                    type="range"
                    min="0"
                    ...
                    onChange={(e) =>
                      setAutoAimSettings((prev) => ({
                        ...prev,
                        minStep: parseFloat(e.target.value),
                      }))
                    }
                    className="w-full h-2 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                  />
                  <p className="text-[9px] text-gray-400 leading-tight">
                    Ignores tiny movements below this. Set to 0 to disable
                    filtering.
                  </p>
                </div>
                ...
                {autoAimSettings.useAbsolute && (
                  <>
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase tracking-wider text-gray-400">
                        <span className="text-green-400">[SNAP]</span> Snap
                        Gain: {autoAimSettings.snapGain.toFixed(2)}
                      </label>
                      <input
                        type="range"
                        min="0.2"
                        ...
                        onChange={(e) =>
                          setAutoAimSettings((prev) => ({
                            ...prev,
                            snapGain: parseFloat(e.target.value),
                          }))
                        }
                        className="w-full h-2 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                      />
                      <p className="text-[9px] text-gray-400 leading-tight">
                        Movement multiplier. Keep ≤1.0 to prevent overshoot.
                        &lt;0.9 undershoots, &gt;1.0 overshoots.
                      </p>
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] uppercase tracking-wider text-gray-400">
                        <span className="text-green-400">[SNAP]</span> Snap Max
                        (°): {autoAimSettings.snapMaxDeg.toFixed(1)}
                      </label>
                      <input
                        type="range"
                        min="5"
                        ...
                        onChange={(e) =>
                          setAutoAimSettings((prev) => ({
                            ...prev,
                            snapMaxDeg: parseFloat(e.target.value),
                          }))
                        }
                        className="w-full h-2 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                      />
                      <p className="text-[9px] text-gray-400 leading-tight">
                        Maximum degrees to move per update. Safety limit for
                        large jumps.
                      </p>
                    </div>
                  </>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-gray-400">
                    <span className="text-green-400">[SNAP]</span> Replan
                    Threshold (°): {autoAimSettings.replanErrorDeg.toFixed(1)}
                  </label>
                  <input
                    type="range"
                    min="0"
                    ...
                    onChange={(e) =>
                      setAutoAimSettings((prev) => ({
                        ...prev,
                        replanErrorDeg: parseFloat(e.target.value),
                      }))
                    }
                    className="w-full h-2 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                  />
                  <p className="text-[9px] text-gray-400 leading-tight">
                    Won't send new commands during movement if error is below
                    this. Higher = smoother/less overshoot, Lower = more
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-gray-400">
                    <span className="text-yellow-400">[BOTH]</span> Deadzone
                    (°): {autoAimSettings.deadzone.toFixed(2)}
                  </label>
                  <input
                    type="range"
                    min="0.1"
                  ...
                  />
                  <p className="text-[9px] text-gray-400 leading-tight">
                    Stops moving when target is within this range. Increase to
                    reduce jitter/oscillation.
                  </p>
                </div>

                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-gray-400">
                    <span className="text-yellow-400">[BOTH]</span> Command
                    Interval (ms): {autoAimSettings.intervalMs}
                  </label>
                  <input
                    type="range"
                    min="50"
                  ...
                  />
                  <p className="text-[9px] text-gray-400 leading-tight">
                    Minimum time between commands. Lower = more responsive but
                    may overload system.
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-gray-400">
                    <span className="text-yellow-400">[BOTH]</span> Smoothing:{" "}
                    {autoAimSettings.smoothing.toFixed(2)}
                  </label>
                  <input
                    type="range"
                    min="0.05"
                  ...
                  />
                  <p className="text-[9px] text-gray-400 leading-tight">
                    Error filter strength. Higher = reacts faster to changes but
                    less stable. Lower = smoother but laggy.
                  </p>
                </div>
              </div>
            </div>
                            ...
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs opacity-70">
                                  {fpsInfo[modelKey]?.current_fps?.toFixed(1) ||
                                    modelInfo.current_fps?.toFixed(1) ||
                                    modelInfo.default_fps}{" "}
                                  FPS
                                </span>
                              </div>
                ...
                {modelsLoaded && Object.keys(availableModels).length > 0 && (
                  <div className="border-t border-gray-600 pt-3 space-y-3">
                    ...
                    {Object.entries(availableModels).map(
                      ([modelKey, modelInfo]) => {
                        const currentFps =
                          fpsInfo[modelKey]?.current_fps ||
                          modelInfo.current_fps ||
                          modelInfo.default_fps;
                        ...
                        return (
                          <div
                            key={modelKey}
                            className={`space-y-2 p-2 rounded ${isActive ? "bg-cyan-900/30 border border-cyan-600/30" : "bg-gray-800/30"}`}
                          >
                            <div className="flex items-center justify-between">
                              <span
                                className={`text-xs font-mono uppercase tracking-wider ${isActive ? "text-cyan-400" : "text-gray-400"}`}
                              >
                                {modelInfo.name}
                              </span>
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-xs font-mono ${isActive ? "text-cyan-400" : "text-gray-400"}`}
                                >
                                  {typeof currentFps === "number"
                            ...
                            <div className="space-y-1">
                              <input
                                ...
                                value={currentFps}
                                onChange={(e) =>
                                  handleFpsChange(modelKey, e.target.value)
                                }
              ...
              {messages && messages.length ? (
                <ul className="space-y-2 text-xs text-gray-200">
                  {messages.map((msg, idx) => (
                    <li
                      key={`${idx}-${msg}`}
                      className="p-2 bg-gray-800/60 rounded border border-gray-700/70"
                    >
                      {msg}
                    </li>
                  ))}
      ...
      <div className="absolute top-4 right-8 z-30 flex flex-col gap-3 items-end pointer-events-auto">
        ...
        <div className="flex items-center gap-2 bg-black/40 border border-cyan-500/30 px-3 py-1.5 rounded-sm text-xs uppercase font-mono tracking-wider backdrop-blur-sm mt-3">
          <span>SYS-LINK:</span>
          <div
            className={`w-3 h-3 rounded-full ${connected ? "bg-cyan-400 animate-pulse" : connecting ? "bg-yellow-400" : "bg-red-500"}`}
          />
          <span
            className={`${connected ? "text-cyan-400" : connecting ? "text-yellow-400" : "text-red-500"}`}
          >
        ...
        {connected && (
          <div className="bg-black/40 border border-cyan-500/30 px-3 py-2 rounded-sm backdrop-blur-sm">
            <h4 className="text-xs uppercase font-mono tracking-wider text-cyan-400 mb-1">
              Hotkeys
            </h4>
            <div className="text-xs font-mono text-gray-400 space-y-0.5">
              <div>
                <span className="text-cyan-300">F/SPACE:</span> Single Fire
              </div>
              <div>
                <span className="text-cyan-300">B/V:</span> Burst Fire
              </div>
              <div>
                <span className="text-cyan-300">R:</span> Home
              </div>
              <div>
                <span className="text-cyan-300">WASD/Arrows:</span> Angular Move
              </div>
              <div>
                <span className="text-cyan-300">H/Home:</span> Center Position
              </div>
              <div>
                <span className="text-cyan-300">1-6:</span> Step Size
              </div>
              {Object.keys(availableModels).length > 0 && (
                <>
                  <div>
                    <span className="text-cyan-300">Shift+1:</span> MobileNet
                    Model
                  </div>
                  <div>
                    <span className="text-cyan-300">Shift+2:</span> YOLO Model
                  </div>
                </>
              )}
            </div>
        ...
        {aiMode && modelsLoaded && (
          <div className="bg-black/40 border border-green-500/30 px-3 py-2 rounded-sm backdrop-blur-sm">
            <h4 className="text-xs uppercase font-mono tracking-wider text-green-400 mb-1">
              AI Detection
            </h4>
            <div className="text-xs font-mono text-gray-400 space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-green-300">
                  {availableModels[currentModel]?.name || currentModel} Active
                </span>
              </div>
              {autoAimEnabled && (
                <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-green-300">Target FPS:</span>{" "}
                {availableModels[currentModel]?.fps || "N/A"}
              </div>
          ...
          {streamError ? (
            <div className="w-full h-full flex items-center justify-center text-center p-8 bg-black/80">
              <div>
                ...
                <h3 className="text-xl font-bold mb-2 text-red-400 uppercase tracking-widest">
                  Video Feed Failure
                </h3>
            ...
            <>
              ...
              {isStreamLoading && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/50">
                  ...
                  <p className="mt-4 text-lg text-cyan-400 tracking-widest">
                    SYNCHRONIZING FEED...
                  </p>
                </div>
              )}
              ...
              {recentlyFired && !triggerActive && (
                <div className="absolute top-4 left-4 bg-orange-900/70 border border-orange-500/50 px-2 py-1 rounded-sm text-xs font-mono text-orange-300 animate-fadeIn">
                  ROUND FIRED:{" "}
                  {Math.floor((3000 - timeSinceLastFire) / 1000) + 1}s
                </div>
              )}
            </>
          )}
